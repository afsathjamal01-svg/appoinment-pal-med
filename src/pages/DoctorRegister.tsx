...
const DoctorRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (userId) {
      ...
      await supabase.from("user_roles").insert({ user_id: userId, role: "doctor" as const });
      await supabase.from("doctors").insert({ user_id: userId, specialization: specialty, experience: Number(experience) || 0 });
    }
    ...
    <div className="flex min-h-screen">
      <div className="hidden w-1/2 items-center justify-center bg-emerald-600 lg:flex">
        <div className="max-w-md px-8 text-center">
          ...
          <h2 className="mt-6 text-3xl font-bold text-white">Doctor Registration</h2>
          <p className="mt-3 text-white/80">Register your practice and start accepting appointments on MediBook.</p>
        </div>
      </div>
      <div className="flex w-full items-center justify-center px-6 lg:w-1/2">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-2xl font-bold text-foreground">Register as Doctor</h1>
          <p className="mt-1 text-sm text-muted-foreground">Fill in your professional details</p>
          <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-xl border bg-card p-6 shadow-sm">
            <div className="space-y-2">
              <Label>Full Name</Label>
              <Input placeholder="Dr. Jane Smith" value={name} onChange={(e) => setName(e.target.value)} required />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="e.g. Cardiologist" value={specialty} onChange={(e) => setSpecialty(e.target.value)} required />
            </div>
            <div className="space-y-2">
              <Label>Experience (years)</Label>
              <Input type="number" placeholder="10" value={experience} onChange={(e) => setExperience(e.target.value)} required />
            </div>
            <div className="space-y-2">
            ...
            <Button className="h-11 w-full bg-emerald-600 hover:bg-emerald-700" type="submit" disabled={loading}>
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Register as Doctor
            </Button>
          </form>
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Already have an account? <Link to="/login/doctor" className="font-medium text-primary hover:underline">Sign in</Link>
          </p>
